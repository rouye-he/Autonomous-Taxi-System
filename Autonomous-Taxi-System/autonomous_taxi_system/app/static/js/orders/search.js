...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.clearAllSearchParams = function() {
        // 检查是否有AJAX分页实例
        if (window.orderPagination) {
            // 获取索引URL并使用AJAX加载
            const indexUrl = document.getElementById('indexUrl').getAttribute('data-url');
    ...
    if (viewToggleBtns.length > 0) {
        viewToggleBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                localStorage.setItem('orderViewType', viewType);
        ...
        // 初始化视图模式
        const savedViewType = localStorage.getItem('orderViewType') || 'table';
